...
public class ADSPONDecoder: Decoder {
    ...
    /**
     ...
     - Parameters:
         ...
         - data: The SPON encoded string to decode.
     - Remark: To retrieve `UIColors` in the record use the `String.uiColor` extension property and to retrieve `UIImages` use the `String.uiImage` extension property.
    ...
    public func decode<T : Decodable>(_ type: T.Type, from data: String) throws -> T {
        ...
        // Unbox and return
        guard let value = try unbox(data, as: T.self) else {
    ...
    public func container<Key>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> where Key : CodingKey {
        ...
        let container = ADSPONKeyedDecodingContainer<Key>(referencing: self, wrapping: topContainer)
    ...
    public func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        return ADSPONUnkeyedDecodingContainer(referencing: self, wrapping: topContainer)
    }
    ...
    public func singleValueContainer() throws -> SingleValueDecodingContainer {
        return ADSPONSingleValueDecodingContainer(referencing: self)
    }
    ...
    internal func unbox(_ value: Any, as type: Date.Type) throws -> Date? {
        ...
        case .iso8601:
            if #available(OSX 10.12, iOS 10.0, watchOS 3.0, tvOS 10.0, *) {
                ...
                guard let date = ADSPONDecoder.iso8601Formatter.date(from: string) else {
